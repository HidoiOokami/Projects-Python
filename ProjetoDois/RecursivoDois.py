...
def fibonacci(quantidade, sequencia=(0, 1)): # a Tupla e para novas sequecias da recursividade
    ...
    # a barra e para quebrar linhas
    return sequencia if len(sequencia) == quantidade else fibonacci(quantidade, sequencia + (sum(sequencia[-2:]),)) #Acrecentando na sequencia atual a soma dos dois ultimos valores 
